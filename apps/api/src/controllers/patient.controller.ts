...
// ─── Liste des patients (ASC / Médecin) ───────────────────
...
export async function getPatientByQrCodeController(
  req: Request,
  res: Response
): Promise<void> {
  try {
...
// ─── Mise à jour du profil patient connecté ───────────────
...
export async function exportDossierController(
  req: AuthRequest,
  res: Response
): Promise<void> {
  try {
